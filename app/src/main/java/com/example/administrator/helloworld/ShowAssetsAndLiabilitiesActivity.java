...
public class ShowAssetsAndLiabilitiesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor = db.rawQuery("select name,money from assets_and_liabilities where type='资产' order by money desc",null);
        ...
        assets.setText(""+df.format(assetsMoney));

        //负债详情
        Double liabilitiesMoney=0.0;
        liabilitiesDetail = (LinearLayout) findViewById(R.id.liabilitiesDetail);
        cursor = db.rawQuery("select name,money from assets_and_liabilities where type='负债' order by money desc",null);
        for (int i = 0; i < cursor.getCount(); i++){
            cursor.moveToNext();
